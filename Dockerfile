...
# To build:
# 
#     docker build -t concrete-base .
#
# To run:
#
#     docker run --rm concrete-base --help
#
# To generate source code for a concrete library repository that
# expects the schema to be located at ~/concrete or ../concrete:
#
#     docker run --mount type=bind,src=$PWD,dst=/app --rm concrete-base
#
...
# concrete-base container:
#
#     docker run --entrypoint= --rm -it concrete-base bash
#
...
RUN curl -O http://archive.apache.org/dist/thrift/0.17.0/thrift-0.17.0.tar.gz && \
